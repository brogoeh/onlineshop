<?php
    ...
    if($banner_id != "")
    {
        ...
        $queryBanner = mysqli_query($links, "SELECT * FROM banner WHERE banner_id='$banner_id'");
        $row=mysqli_fetch_array($queryBanner);
           
		$banner = $row["banner"];
		$link = $row["link"];
		$gambar = "<img src='". BASE_URL."images/slides/$row[gambar]' style='width: 200px;vertical-align: middle;' />";
...
<form action="<?php echo BASE_URL."module/banner/action.php?banner_id=$banner_id"?>" method="post" enctype="multipart/form-data">
	...
	<div class="el-form">
		<span><input type="submit" name="button" id="button" value="<?php echo $button; ?>" /></span>
	</div>	
</form>